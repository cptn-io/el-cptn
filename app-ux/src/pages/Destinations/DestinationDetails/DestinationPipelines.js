...
const DestinationPipelines = (props) => {
    ...
    useEffect(() => {
        axios.get(`/api/destination/${destinationId}/pipeline?page=${page}`).then(response => {
            setTotalCount(response.headers['x-total-count'] || 0);
            setData(response.data);
        }).catch(err => {
            addNotification({
                message: get(err, 'response.data.message', 'An error occurred while fetching Destination Pipelines'),
                type: 'error'
            });
        }).finally(() => {
            setLoading(false);
